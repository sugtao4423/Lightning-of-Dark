...
class _3_follow : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val userFollow = v.findViewById<TweetListView>(R.id.userPageList)

        adapter = TweetListUserAdapter(activity!!)
        userFollow.adapter = adapter

        val scrollListener = getLoadMoreListener(userFollow.linearLayoutManager)
        ...
        object : AsyncTask<Unit, Unit, PagableResponseList<User>?>() {
            ...
            override fun onPostExecute(result: PagableResponseList<User>?) {
                if (result != null) {
                    ...
                    if (targetUser != null && targetUser!!.friendsCount <= adapter.itemCount) {
                        isAllLoaded = true
                    }
                } else {
