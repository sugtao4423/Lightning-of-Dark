...
public class Dialog_talk implements OnClickListener{
    ...
    public void onClick(View v){
        ...
        reply = status.isRetweet() ? status.getRetweetedStatus() : status;
    ...
    private class LoadConversation extends AsyncTask<Void, Void, Status>{
        ...
        protected twitter4j.Status doInBackground(Void... params){
            try{
                reply = ((App)context.getApplicationContext()).getTwitter().showStatus(reply.getInReplyToStatusId());
                return reply;
        ...
        public void onPostExecute(twitter4j.Status result){
            if(result != null){
                ...
                if(result.getInReplyToStatusId() > 0){
                    ...
                    new ShowToast(context.getApplicationContext(), R.string.success_get_talk_list);
                }
            }else{
                new ShowToast(context.getApplicationContext(), R.string.error_get_talk_list);
            }
        }
    }
